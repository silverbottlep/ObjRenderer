...
void processModel(const std::string &rootPath, const std::string &filePath,
                  const Args &args) {
  ...
  std::string viewFolderPath =
      rootPath + filePath.substr(0, filePath.length() - 4) + "_views";
  ...
  for (int theta = 0; theta < 360; theta += args.theta_inc) {
    for (int phi = 0; phi <= args.phi_max; phi += args.phi_inc) {
      ...
      unsigned int filterSize =
          round(double(args.render_size) / args.output_size) * 2 + 1;
      ...
      for (int i = 0; i < aa_image.rows; i++) {
        for (int j = 0; j < aa_image.cols; j++) {
          ...
          aa_image.at<cv::Vec4f>(i, j) =
              cv::Vec4f(c[0] * args.brightness, c[1] * args.brightness,
                        c[2] * args.brightness, c[3]);
        }
      }

      sprintf(fn, "%s/render_%03d_%02d.png", viewFolderPath.c_str(), theta,
              phi);
      ...
      if (args.output_coord) {
        ...
        image = ObjRenderer::genShading();
        cv::GaussianBlur(image, image, cv::Size(filterSize, filterSize), 0, 0);
        cv::resize(image, aa_image,
                   cv::Size(args.output_size, args.output_size));

        sprintf(fn, "%s/cord_%03d_%02d.exr", viewFolderPath.c_str(), theta,
                phi);
        ...
        for (int i = 0; i < aa_image.rows; i++) {
          for (int j = 0; j < aa_image.cols; j++) {
            const cv::Vec4f &c =
                aa_image.at<cv::Vec4f>(aa_image.rows - i - 1, j);
      ...
      if (args.output_norm) {
        ...
        sprintf(fn, "%s/norm_%03d_%02d.exr", viewFolderPath.c_str(), theta,
                phi);
...
void genViews(const std::string envPath, const std::string &folderPath,
              const Args &args) {
