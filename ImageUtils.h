...
cv::Mat fi2mat(FIBITMAP *src) {
  ...
  // FIT_RGBAF     //128 - bit RGBA float image : 4 x 32 - bit IEEE floating
  // point
  ...
  switch (fit) {
  case FIT_UINT16:
    cv_type = cv::DataType<ushort>::type;
    break;
  case FIT_INT16:
    cv_type = cv::DataType<short>::type;
    break;
  case FIT_UINT32:
    cv_type = cv::DataType<unsigned>::type;
    break;
  case FIT_INT32:
    cv_type = cv::DataType<int>::type;
    break;
  case FIT_FLOAT:
    cv_type = cv::DataType<float>::type;
    break;
  case FIT_DOUBLE:
    cv_type = cv::DataType<double>::type;
    break;
  case FIT_COMPLEX:
    cv_type = cv::DataType<cv::Complex<double>>::type;
    break;
  case FIT_RGB16:
    cv_type = cv::DataType<cv::Vec<ushort, 3>>::type;
    cv_cvt = CV_RGB2BGR;
    break;
  case FIT_RGBA16:
    cv_type = cv::DataType<cv::Vec<ushort, 4>>::type;
    cv_cvt = CV_RGBA2BGRA;
    break;
  case FIT_RGBF:
    cv_type = cv::DataType<cv::Vec<float, 3>>::type;
    cv_cvt = CV_RGB2BGR;
    break;
  case FIT_RGBAF:
    cv_type = cv::DataType<cv::Vec<float, 4>>::type;
    cv_cvt = CV_RGBA2BGRA;
    break;
  case FIT_BITMAP:
    switch (bpp) {
    case 8:
      cv_type = cv::DataType<cv::Vec<uchar, 1>>::type;
      break;
    case 16:
      cv_type = cv::DataType<cv::Vec<uchar, 2>>::type;
      break;
    case 24:
      cv_type = cv::DataType<cv::Vec<uchar, 3>>::type;
      break;
    case 32:
      cv_type = cv::DataType<cv::Vec<uchar, 4>>::type;
      break;
    default:
...
template <typename T> cv::Mat filter_envmap(const cv::Mat &map) {
  cv::Mat filtered;
  map.copyTo(filtered);
